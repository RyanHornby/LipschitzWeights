...
syn_normal = function(origonal_data, model_string = "outcome ~ 1", chains = 1, iterations = 1000,
  ...
  for (i in 1:m){
    draws_exp_mean <- exp(draws[start + thin * (i - 1), 1])
    draws_sd <- draws[start + thin * (i - 1), 2]
