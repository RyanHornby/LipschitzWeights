...
syn_Gamma = function(origonal_data, model_string = "outcome ~ 1", chains = 1, iterations = 1000,
  ...
  for (i in 1:m){
    ...
    shape = draws[start + thin * (i - 1), 2]
    scale = shape / draws_exp
    syndata[[i]] <- rgamma(N, shape, scale)
  }
